...
impl PrivateKey {
    ...
    pub fn generate_elevenchain(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "elevenchain_helpers")]
    pub fn generate_elevenchain_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_elevenchain(rng))
            .collect()
    }
