...
const char * const BITCOIN_CONF_FILENAME = "elevenchain.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //elevenchain-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\elevenchain
    // macOS: ~/Library/Application Support/elevenchain
    // Unix-like: ~/.elevenchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "elevenchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/elevenchain";
#else
    // Unix-like
    return pathRet / ".elevenchain";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty elevenchain.conf if it does not exist
