...
The elevenchain Core daemon can be configured to act as a trusted "border
router", implementing the elevenchain wire protocol and relay, making
...
The ZeroMQ feature in elevenchain Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building elevenchaind:
...
For instance:
    ...
    $ elevenchaind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/elevenchaind.tx.raw \
...
These options can also be provided in elevenchain.conf.
...
From the perspective of elevenchaind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into elevenchaind directly. Furthermore, no information is
...
using. elevenchaind appends an up-counting sequence number to each
