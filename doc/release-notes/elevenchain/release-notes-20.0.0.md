# elevenchain Core version v20.0.0
...
Currently, 10% of block rewards are set aside for the elevenchain DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking elevenchain in a
credit pool. This credit pool mirrors the available credit supply in elevenchain
Platform. When a certain amount of elevenchain is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on elevenchain
Platform. Masternode identities receive the fees and can convert them back to
elevenchain by withdrawing them from elevenchain Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the echz locked in all previous
  blocks minus all the echz unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from elevenchain Core (BIP61)

This feature has been disabled by default since elevenchain Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since elevenchain Core version 20.0.0 there are extra changes:
...
- It is possible to run elevenchain Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `ECH/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from elevenchain Core (BIP61)_](#removal-of-reject-network-messages-from-elevenchain-core-bip61) above for
...
- The `elevenchain-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `elevenchain-cli` tool both take advantage of this, so
for example:

```sh
elevenchain-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
elevenchain-cli -named createwallet mywallet load_on_startup=1
```
...
with elevenchain’s product needs, such as SegWit and RBF, are excluded from our
...
- [v19.3.0](https://github.com/Elevenchain/blob/master/doc/release-notes/elevenchain/release-notes-19.3.0.md) released July/31/2023
...
[DIP-0023]: https://github.com/elevenchainpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/elevenchainpay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/Elevenchain/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/Elevenchain/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/Elevenchain/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/Elevenchain/compare/v19.3.0...elevenchainpay:v20.0.0
