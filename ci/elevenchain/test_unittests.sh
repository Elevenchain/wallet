...
source ./ci/elevenchain/matrix.sh
...
cd build-ci/elevenchain-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_elevenchain.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_elevenchain --catch_system_errors=no -l test_suite
  else
